      ...
      textarea: {
        element : function(object, attrs) {   
          ...
          var newObject = $.fn.editableSet.attributor( $('<textarea />'), attrs );
					...
					//htmldecode, then replace BRs with newlines
					newObject.val( $.htmlDecode(attrs.value.replace( /<br.*?>/ig, "\r\n" )) ); 
          $(object).replaceWith( newObject );
        }
      },
      ...
      if( fieldName ) {
        ...
        // Assign the determined value to the span   
				if( typeof value !== undefined ) {
					spanObj = $(span);					
					if (spanObj.attr("data-type") == "textarea") {
						//htmlencode, then replace newlines with BRs
						spanObj.html( $.htmlEncode(value).replace(/(\r\n|[\r\n])/g, "<br />") );
					} 
					else {
          	spanObj.text( value );
					}
